...
RESDIR = '/media/data/mimic/hf_admdata_99p'
SOURCEDIR = '/media/data/mimic/admdata_valid/'
LABELDIR = '/media/data/mimic/admdata_timelabels/'
PROCESSED_DB_DIR = '/media/data/mimic/hf_admdata_17f/processed_db/'
...
tempdball = np.load(os.path.join(RESDIR, '../hf_admdata_17f/24hrs/DB_merged_24hrs.npy'), allow_pickle=True)
...
np.save(os.path.join('/media/data/mimic/hf_admdata_99p/raw/', 'map_feature_colids.npy'), map_feature_colids)
...
def merge_items(timeseries, mergenode, new_column_id, keep_func=keep_nonneg):
    ...
    if type(mergenode) == type(()):
        ...
        if mergename.endswith('_div'):
            ...
            merge_func = np.nanmaxallidsallids
