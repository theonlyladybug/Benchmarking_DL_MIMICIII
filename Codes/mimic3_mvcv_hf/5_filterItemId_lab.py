...
np.save(f'{res_dir}/filtered_lab_raw.npy', {'raw': results})
print('saved!')
...
dropped_id = []
multiple_units = []
results = np.load(f'{res_dir}/filtered_lab_raw.npy', allow_pickle=True).tolist()['raw']
for x in results:
...
np.save(f'{res_dir}/filtered_lab.npy',{'id':valid_lab,'unit':valid_lab_unit})
print('saved!')
...
# for d in leftids:
    print('LAB : ' + str(d))
    cur = conn.cursor()
    cur.execute('SELECT value, valueuom, count(*) as x FROM mimiciii.labevents as lb                 WHERE itemid = '+ str(d) +' and hadm_id in (select * from admission_ids) GROUP BY value, valueuom ORDER BY x DESC')
...
np.save(f'{res_dir}/lab_dropped_value.npy', dropped_value)


# In[10]:


dropped_value = np.load(f'{res_dir}/lab_dropped_value.npy', allow_pickle=True).tolist()
valid_lab_num = []
valid_lab_num_unit = []
valid_lab_cate = []
valid_lab_ratio = []
...
np.save(f'{res_dir}/filtered_lab_num',{'id':valid_lab_num,'unit':valid_lab_num_unit})
np.save(f'{res_dir}/filtered_lab_cate',{'id':valid_lab_cate,'unit':None})
np.save(f'{res_dir}/filtered_lab_ratio', {'id': valid_lab_ratio, 'unit': None})
