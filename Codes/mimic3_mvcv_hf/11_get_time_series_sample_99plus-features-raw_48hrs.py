...
def try_making_splits(y, nfold, test_ratio=None):
    ...
    while i_trial < nfold:
        ...
        while cnt2 < 1000:
            ...
            for idx_tr_ss2, idx_va_ss2 in ss2.split(np.zeros(len(y[idx_trva, i_class_least])), y[idx_trva, i_class_least]):  # length is only 1 !
...
working_path = '/media/data/mimic/hf_admdata_99p/%dhrs_raw/' % HRS
...
LAB_EVENTS_IDX = np.array(list(range(40,73))+list(range(79, 102))+list(range(102, 110))+list(range(111, 117))+list(range(117, 129))+list(range(138, 140)))
...
old_map_feature_colids = np.load(os.path.join('/media/data/mimic/hf_admdata_99p/raw/', 'map_feature_colids.npy'), allow_pickle=True).tolist()
map_feature_colids = {}
...
sum_list = list(map(lambda x: map_feature_colids[x], ['urinary_output_sum']))
# adm_features_all
total_featuren = len(minmaxavg_list)*3 + len(sum_list)*1 + adm_features_all.shape[1]
...
#         X_imputed[i_n][np.isnan(X_imputed[i_n])] = 0
    ...
    for i_n in range(N):
        ...
        ep_tdata_masking[i_n, :tt_min, :] =             (~np.isnan(xx_mis[:tt_min, :])).astype(int)
    print('done!')
