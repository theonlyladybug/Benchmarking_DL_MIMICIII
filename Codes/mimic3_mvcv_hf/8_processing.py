...
adm_dir = "/media/data/mimic/admdata"
...
v = np.load(f'{res_dir}/filtered_input.npy', allow_pickle=True).tolist()
valid_input = v['id']
valid_input_unit = v['unit']

v = np.load(f'{res_dir}/filtered_output.npy', allow_pickle=True).tolist()
valid_output = v['id']

v = np.load(f'{res_dir}/filtered_chart.npy', allow_pickle=True).tolist()
valid_chart = v['id']
valid_chart_unit = v['unit']

v = np.load(f'{res_dir}/filtered_chart_num.npy', allow_pickle=True).tolist()
valid_chart_num = v['id']
valid_chart_num_unit = v['unit']

v = np.load(f'{res_dir}/filtered_chart_cate.npy', allow_pickle=True).tolist()
valid_chart_cate = v['id']

v = np.load(f'{res_dir}/filtered_chart_ratio.npy', allow_pickle=True).tolist()
valid_chart_ratio = v['id']

v = np.load(f'{res_dir}/filtered_lab.npy', allow_pickle=True).tolist()
valid_lab = v['id']
valid_lab_unit = v['unit']

v = np.load(f'{res_dir}/filtered_lab_num.npy', allow_pickle=True).tolist()
valid_lab_num = v['id']
valid_lab_num_unit = v['unit']

v = np.load(f'{res_dir}/filtered_lab_cate.npy', allow_pickle=True).tolist()
valid_lab_cate = v['id']

v = np.load(f'{res_dir}/filtered_lab_ratio.npy', allow_pickle=True).tolist()
valid_lab_ratio = v['id']

v = np.load(f'{res_dir}/filtered_microbio.npy', allow_pickle=True).tolist()
valid_microbio = v['id']

v = np.load(f'{res_dir}/filtered_prescript.npy', allow_pickle=True).tolist()
...
for itemidlist, itemidlistname in zip([valid_input, valid_output, valid_chart, valid_chart_num, valid_chart_cate, valid_chart_ratio, valid_lab, valid_lab_num, valid_lab_cate, valid_lab_ratio], 'valid_input, valid_output, valid_chart, valid_chart_num, valid_chart_cate, valid_chart_ratio, valid_lab, valid_lab_num, valid_lab_cate, valid_lab_ratio'.replace(' ', '').split(',')):
    ...
    cur.execute(sql)
    conn.commit()
    sql = 'create table if not exists mengcztemp_itemids_{0} (    itemid serial PRIMARY KEY     )'.format(itemidlistname)
    cur.execute(sql)
    conn.commit()
...
cur.execute(sql)
conn.commit()
sql = 'create table if not exists mengcztemp_itemids_{0} (    itemid varchar(255) PRIMARY KEY     )'.format('valid_prescript')
cur.execute(sql)
conn.commit()
...
np.save(f'{res_dir}/map_itemid_index.npy', map_itemid_index)
...
np.save(f'{res_dir}/catedict.npy', catedict)
print('saved!')
...
catedict = np.load(f'{res_dir}/catedict.npy', allow_pickle=True).tolist()
# print(catedict)
...
np.save(f'{res_dir}/adm_catemappings.npy', mapping)
mapping
...
if os.path.exists(f'{adm_dir}/'):
    shutil.rmtree(f'{adm_dir}/')
os.makedirs(f'{adm_dir}/log/')
...
def process_patient(aid):
    with open(f'{adm_dir}/log/adm-{0}.log'.format(str('%.6d' % aid)), 'w') as f:
        try:
            ...
            np.save(f'{adm_dir}/adm-' + str('%.6d' % aid), res)
#             print('finished {0}!'.format(aid))
        except Exception as e:
            with open(f'{adm_dir}/log/admerror-{0}.log'.format(str('%.6d' % aid)), 'w') as ferr:
...
def add_mortality_labels(aid):
    with open(f'{adm_dir}/log/adm-addml{0}.log'.format(str('%.6d' % aid)), 'w') as f:
        try:
            res = np.load(f'{adm_dir}/adm-' + str('%.6d' % aid)+'.npy', allow_pickle=True).tolist()
        except Exception as e:
            traceback.print_exc(file=f)
            return
        res['general'] = ageLosMortality(aid, f)
        np.save(f'{adm_dir}/adm-' + str('%.6d' % aid)+'.npy', res)
#     print('finished {0}!'.format(aid))
