...
cur = conn.cursor()
# sapsii
with open(os.path.join(working_dir, 'concepts/severityscores/sapsii.sql'), 'r') as f:
    cur.execute("SET search_path TO mimiciii;" + f.read())
    conn.commit()

# sofa
with open(os.path.join(working_dir, 'concepts/severityscores/sofa.sql'), 'r') as f:
    cur.execute("SET search_path TO mimiciii;" + f.read())
    conn.commit()
...
def get_sapsii(aid):
    conn = getConnection()
    sql = 'select * from mimiciii.sapsii where hadm_id = {0}'.format(aid)
    cur = conn.cursor()
    cur.execute(sql)
    res = cur.fetchone()
    if res:
...
def get_sofa_score(aid):
    conn = getConnection()
    sql = 'select sofa from mimiciii.sofa where hadm_id = {0}'.format(aid)
    cur = conn.cursor()
    cur.execute(sql)
    res = cur.fetchone()
    if res:
