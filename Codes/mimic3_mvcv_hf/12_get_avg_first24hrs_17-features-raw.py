...
# mortality labels
adm_labels_all = np.load(os.path.join(SERIALDIR, 'imputed-normed-ep_1_%d.npz' % HRS), allow_pickle=True)['adm_labels_all']
with open(os.path.join(RESDIR, 'output.csv'), 'w') as f:
    for res in adm_labels_all:
        f.write(','.join(list(map(str, res))) + '\n')
        
# icd9 labels
# icd9_labels_all = np.load(os.path.join(SERIALDIR, 'imputed-normed-ep_1_%d.npz' % HRS), allow_pickle=True)['y_icd9']
